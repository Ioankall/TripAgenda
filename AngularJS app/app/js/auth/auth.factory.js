...
myApp.factory('UserAuthFactory', function($window, $location, $http, AuthenticationFactory) {
  return {
    ...
    login: function(username, password) {
      return $http.post('http://snf-703110.vm.okeanos.grnet.gr:3000/login', {
        username: username,
        password: password
      });
    },
    register: function(username, password, email, name, role) {
      return $http.post('http://snf-703110.vm.okeanos.grnet.gr:3000/register', {
        username: username,
        password: password,
		email: email,
		name: name,
